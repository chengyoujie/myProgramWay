...
/**存放 lanTxt 变量的类名 便于import时使用 */
let cfgMgrPath = path.join(workSpace, "src/core/manager/CFGManager");
/**代码路径， 用于检测所有的ts文件中汉字 */
let codePath = path.join(workSpace, "src")
/**需要替换的文本 */
var waitReplaceTxt = [];
/**是否需要导入lanTxt声明 */
var needImportLanTxt = null;//{start:0, end:0, txt:}
/**源文件的内容 */
var fileContent = "";
...
const transformAST = function (context){
    return function searchNode(node){
        ...
        if(node.kind == ts.SyntaxKind.ImportDeclaration){
            if(node.moduleSpecifier.text.indexOf("CFGManager") != -1){
                let importElement = [];
                let hasImportLan = false;
                let elements = node.importClause.namedBindings.elements;
                for(let i=0; i<elements.length; i++){
                    if(elements[i].name.escapedText=="lanTxt")hasImportLan = true;
                    importElement.push(elements[i].name.text);
                }
                if(!hasImportLan){
                    importElement.push("lanTxt")
                }
                needImportLanTxt = {hasImportLan:hasImportLan, start:node.pos, end:node.end, txt:`${node.pos==0?"":"\n"}import { ${importElement.join(", ")} } from "${node.moduleSpecifier.text}";`}
            }
        }
        if(node.kind == ts.SyntaxKind.CallExpression)//方法调用
        {
            if(node.expression.kind == ts.SyntaxKind.PropertyAccessExpression)//如果函数是A.b属性调用的 
            {
                ...
                // let callPropName = node.expression.name.text;//获取  A.b 的b字段
        ...
        }else if(node.kind == ts.SyntaxKind.BinaryExpression)//表达式
        {
            if(node.operatorToken && node.operatorToken.kind == ts.SyntaxKind.PlusToken)//A+B形式的
            {
                ...
                if(findText)
                {
                    ...
                    waitReplaceTxt.push({start:node.pos, end:node.end, txt:"pub.StringUtil.substitute(lanTxt.k_"+lanIndex+", "+paramStrs.join(",") +")"});
                    return createCallExpression(
                        createPropertyAccessExpression(
                            factory.createIdentifier("pub.StringUtil"),
...
function checkFile(url)
{
    ...
    waitReplaceTxt.length = 0;
    needImportLanTxt = null;
    ...
    if(waitReplaceTxt.length>0)
    {
        ...
        if(!needImportLanTxt)
        {
            let releatePath = path.relative(url, cfgMgrPath).replace(/\\/gi, "/").replace(/^\.\./gi, ".");
            console.log(releatePath)
            let importLanTxt = `import { lanTxt } from "${releatePath}";\n`;
            newContent = importLanTxt + newContent;
            indexChange += importLanTxt.length;
        }
        if(needImportLanTxt && !needImportLanTxt.hasImportLan){
            let item = needImportLanTxt;
            newContent = newContent.substring(0, item.start+indexChange)+item.txt+newContent.substring(item.end+indexChange);
...
function parserLanObj(lanObjUrl){
    ...
    for(let key in lanObjDic)
    {
        ...
        ////新添加的放到后面， 如果注释掉此段代码会找空的地方插入
        if(maxLanIndex<index)
        {
            maxLanIndex = index+1;
        }
        ////
...
/**检测所有的文件 */
function checkAll(searchUrl){
    if(!searchUrl || searchUrl.length==0){
        searchUrl = codePath;
    }
    walkDir(searchUrl, (url)=>{
           if(path.extname(url)!=".ts")return;
           if(url.indexOf("src\\declare")!=-1 || url.indexOf("src\\ui")!=-1)return;
           run(url);
    })
}
...
//test code
// checkAll()
//checkAll(path.join(workSpace, "src/game/module"));
// run("D:/LayaProject/code/Game/src/game/module/common/AlertGetRwdView.ts")

//导出函数
module.exports = {run, checkAll};
