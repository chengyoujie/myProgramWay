...
//读取本地存取的名对应的
searchFile(path.join(srcpath, "declare/ui"), getClsName);//查找 UI类名
...
// console.log(path.join(__dirname, "UI声明使用次数.txt"));
fs.writeFileSync(path.join(__dirname, "UI声明使用次数.txt"), txt, "utf-8");
fs.writeFileSync(path.join(__dirname, "declareUseTime.html"), html, "utf-8");
...
function getClsName(filepath)
{
    ...
    while(clsArr = clsReg.exec(content)){
        ...
        clsNum ++;
        clsDic[clsName] = {num:0, url:filepath};
    }
