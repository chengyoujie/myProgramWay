...
//变量
var rootpath = "D:/worksvn";
var publishpath = "D:/publish";
...
function doCopy(runNext)
{
    ...
    log("拷贝完成");
    log("开始删除缓存文件")
    rmdirSync(allbinpath);
    log("缓存文件删除完毕")
    if(runNext)
    {
        doSvnCommit(publishpath);
    }else{
        isrun = false;
        userip = "";
...
/**开始提交到发布svn上 */
function doSvnCommit(filePath, endFun=null)
{
    // log("开始提交项目")
    cmdOper("svn stat "+filePath, function(stdout){
        let changeReg = /([\S])\s+(.*?)\s*[\r\n]/g;
        let addFiles = [];
        let modFiles = [];
        let remFiles = [];
...
function oper(ip, operName, data)//{id, code, web, name}
{
    ...
    if(isrun)
    {
        log("<font color='#ff0000'>当前正在编译中.... 【"+userip+"】</font>");
        return false;
    }
    userip = ip;
    doOper(operName, data);
    return true;
}


/**发布入口函数 */
function doOper(operName, data)
{
    ...
    //     return;
    // }
    rootpath = projectData.codepath;
    publishpath = projectData.pubpath;
    log("发布【"+projectData.name+"】版本")
    isrun = true;
    if(operName == "publish")
    {
        doSvnUpdate(true);
    }else if(operName == "copydata2web"){
        doCopyData2Web();
    }
...
function doCopyData2Web(){
    svnUpdate(publishpath, function(stdout, isNotCof1){//更新发布路径
        if(!isNotCof1){
            return;
        }
        let datapath = path.join(projectData.datapath, "/Data/")
        svnUpdate(datapath, function(stdout, isNotCof1){//更新数据路径
            if(!isNotCof1){
                return;
            }
            
            svnUpdate(rootpath, function(stdout, isNotCof2){
                if(!isNotCof2){
                    return;
                }
                let qufuDir = path.join(rootpath, "qufuResource/config");
                let qufuResource = path.join(qufuDir, "randName.json")
                let jsonFile = path.join(datapath, "XmlData/SERVER/filter", "randName.json");
                let toFile = path.join(publishpath, "qufuResource/config", "randName.json");
                fs.copyFileSync(jsonFile, toFile);
                fs.copyFileSync(jsonFile, qufuResource);
                doSvnCommit(qufuDir, function(){
                    doSvnCommit(publishpath);
                    console.log("拷贝完成"+userip)
                    isrun = false;
                });
