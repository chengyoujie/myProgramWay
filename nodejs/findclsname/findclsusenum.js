...
let notDeleCls = [
    "Main",
    "GodMazeTalentView",
    "GodMazeRechargeRender",
    "GodMazeRankRender",
    "GodMazeMainView",
    "GodMazeFightReadyView",
    "LanSwitch"
];
...
for(let i=0; i<clsNums.length; i++)
{
    ...
    if(notDeleCls.indexOf(itemInfo.cls) != -1)continue;
    if(itemInfo.num == 0)
    {
...
console.log(path.join(__dirname, "文本类使用次数.txt"));
fs.writeFileSync(path.join(__dirname, "文本类使用次数.txt"), txt, "utf-8");
fs.writeFileSync(path.join(__dirname, "clsUseTime.html"), html, "utf-8");
...
function findClsNum(filepath, filtertype = "ts")
{
    ...
    dealCount++;
    console.log(dealCount+"/"+totalCount+" 检查——>"+filepath)
}
function findUiClsNum(filepath)
{
    ...
    dealCount++;
    console.log(dealCount+"/"+totalCount+" 保存——>"+filepath)
}
